...
public final class Poll extends AbstractPoll {
    ...
    private static final boolean isPollsProcessing = Prizm.getBooleanProperty("prizm.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, Prizm.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, Prizm.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, Prizm.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, Prizm.getBlockchain().getHeight());
    ...
    public boolean isFinished() {
        return finishHeight <= Prizm.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, Prizm.getBlockchain().getHeight());
        if (countHeight < Prizm.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }
