...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = Prizm.getIntProperty("prizm.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.PRIZM ? holdingId : 0);
        ...
        synchronized(monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.PRIZM || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Prizm.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Prizm.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case PRIZM:
                                        processPrizmEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
    ...
    /**
     * Process a Prizm event
     *
     * @param   monitoredAccount            Monitored account
     * @param   targetAccount               Target account
     * @param   fundingAccount              Funding account
     * @throws  PrizmException                Unable to create transaction
     */
    private static void processPrizmEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = Prizm.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short)1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(Prizm.getBlockchain().getLastBlockTimestamp());
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                monitoredAccount.height = Prizm.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("coins funding transaction %s for %f coins submitted from %s to %s",
                        transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_PRIZM,
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            //
            // Check the Prizm balance for monitored accounts
            //
            synchronized(monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                       if (maccount.monitor.holdingType == HoldingType.PRIZM && balance < maccount.threshold &&
