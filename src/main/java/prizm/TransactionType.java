...
import prizm.Account.ControlType;
import prizm.AccountLedger.LedgerEvent;
import prizm.Attachment.AbstractAttachment;
import prizm.PrizmException.ValidationException;
import prizm.VoteWeighting.VotingModel;
...
public abstract class TransactionType {
    ...
    private static final byte TYPE_COLORED_COINS = 4;
...
//                return ShufflingTransaction.findTransactionType(subtype);
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public final static TransactionType ARBITRARY_MESSAGE = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (transaction.getRecipientId() == Genesis.CREATOR_ID && Prizm.getBlockchain().getHeight() > Constants.MONETARY_SYSTEM_BLOCK) {
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            @Override
            boolean isBlockDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                return Prizm.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK
        ...
        public static final TransactionType ALIAS_SELL = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingAliasSell(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasSell parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                if (transaction.getAmountNQT() != 0) {
                    throw new PrizmException.NotValidException("Invalid sell alias transaction: " +
                ...
                if (priceNQT < 0 || priceNQT > Constants.MAX_BALANCE_NQT) {
                    throw new PrizmException.NotValidException("Invalid alias sell price: " + priceNQT);
                }
                if (priceNQT == 0) {
                    if (Genesis.CREATOR_ID == transaction.getRecipientId()) {
                        throw new PrizmException.NotValidException("Transferring aliases to Genesis account not allowed");
                    } else if (transaction.getRecipientId() == 0) {
                        throw new PrizmException.NotValidException("Missing alias transfer recipient");
                    }
                }
                ...
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new PrizmException.NotValidException("Selling alias to Genesis not allowed");
                }
            }
        ...
        public static final TransactionType ALIAS_BUY = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingAliasBuy(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasBuy parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (alias == null) {
                    ...
                    throw new PrizmException.NotCurrentlyValidException("Alias is owned by account other than recipient: "
                            + Long.toUnsignedString(alias.getAccountId()));
                }
                Alias.Offer offer = Alias.getOffer(alias);
                if (offer == null) {
                    throw new PrizmException.NotCurrentlyValidException("Alias is not for sale: " + aliasName);
                }
                if (transaction.getAmountNQT() < offer.getPriceNQT()) {
                    ...
                    throw new PrizmException.NotCurrentlyValidException(msg);
                }
                if (offer.getBuyerId() != 0 && offer.getBuyerId() != transaction.getSenderId()) {
                    throw new PrizmException.NotCurrentlyValidException("Wrong buyer for " + aliasName + ": "
        ...
        public final static TransactionType POLL_CREATION = new Messaging() {
            ...
            private final Fee POLL_OPTIONS_FEE = new Fee.SizeBasedFee(10 * Constants.ONE_PRIZM, Constants.ONE_PRIZM, 1) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            private final Fee POLL_SIZE_FEE = new Fee.SizeBasedFee(0, 2 * Constants.ONE_PRIZM, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingPollCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingPollCreation(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingPollCreation parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getPollName().length() > Constants.MAX_POLL_NAME_LENGTH
                        ...
                        || optionsCount == 0) {
                    throw new PrizmException.NotValidException("Invalid poll attachment: " + attachment.getJSONObject());
                }

                if (attachment.getMinNumberOfOptions() < 1
                        || attachment.getMinNumberOfOptions() > optionsCount) {
                    throw new PrizmException.NotValidException("Invalid min number of options: " + attachment.getJSONObject());
                }
                ...
                if (attachment.getMaxNumberOfOptions() < 1
                    ...
                    throw new PrizmException.NotValidException("Invalid max number of options: " + attachment.getJSONObject());
                }

                for (int i = 0; i < optionsCount; i++) {
                    if (attachment.getPollOptions()[i].length() > Constants.MAX_POLL_OPTION_LENGTH
                            || attachment.getPollOptions()[i].isEmpty()) {
                        throw new PrizmException.NotValidException("Invalid poll options length: " + attachment.getJSONObject());
                    }
                }
                ...
                if (attachment.getMinRangeValue() < Constants.MIN_VOTE_VALUE || attachment.getMaxRangeValue() > Constants.MAX_VOTE_VALUE
                    ...
                    throw new PrizmException.NotValidException("Invalid range: " + attachment.getJSONObject());
                }
                ...
                if (attachment.getFinishHeight() <= attachment.getFinishValidationHeight(transaction) + 1
                    ...
                    throw new PrizmException.NotCurrentlyValidException("Invalid finishing height" + attachment.getJSONObject());
                }

                if (! attachment.getVoteWeighting().acceptsVotes() || attachment.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                    throw new PrizmException.NotValidException("VotingModel " + attachment.getVoteWeighting().getVotingModel() + " not valid for regular polls");
                }
        ...
        public final static TransactionType VOTE_CASTING = new Messaging() {
            ...
            @Override
            Attachment.MessagingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingVoteCasting parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getPollId() == 0 || attachment.getPollVote() == null
                    ...
                    throw new PrizmException.NotValidException("Invalid vote casting attachment: " + attachment.getJSONObject());
                }
                ...
                if (poll == null) {
                    throw new PrizmException.NotCurrentlyValidException("Invalid poll: " + Long.toUnsignedString(attachment.getPollId()));
                }

                if (Vote.getVote(pollId, transaction.getSenderId()) != null) {
                    throw new PrizmException.NotCurrentlyValidException("Double voting attempt");
                }

                if (poll.getFinishHeight() <= attachment.getFinishValidationHeight(transaction)) {
                    throw new PrizmException.NotCurrentlyValidException("Voting for this poll finishes at " + poll.getFinishHeight());
                }
                ...
                for (byte vote : votes) {
                    if (vote != Constants.NO_VOTE_VALUE && (vote < poll.getMinRangeValue() || vote > poll.getMaxRangeValue())) {
                        throw new PrizmException.NotValidException(String.format("Invalid vote %d, vote must be between %d and %d",
                ...
                if (positiveCount < poll.getMinNumberOfOptions() || positiveCount > poll.getMaxNumberOfOptions()) {
                    throw new PrizmException.NotValidException(String.format("Invalid num of choices %d, number of choices must be between %d and %d",
        ...
        public static final TransactionType PHASING_VOTE_CASTING = new Messaging() {
            ...
            private final Fee PHASING_VOTE_FEE = (transaction, appendage) -> {
                ...
                return attachment.getTransactionFullHashes().size() * Constants.ONE_PRIZM;
            };
            ...
            @Override
            Attachment.MessagingPhasingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingPhasingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingPhasingVoteCasting parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (revealedSecret.length > Constants.MAX_PHASING_REVEALED_SECRET_LENGTH) {
                    throw new PrizmException.NotValidException("Invalid revealed secret length " + revealedSecret.length);
                }
                ...
                if (hashes.size() > Constants.MAX_PHASING_VOTE_TRANSACTIONS) {
                    throw new PrizmException.NotValidException("No more than " + Constants.MAX_PHASING_VOTE_TRANSACTIONS + " votes allowed for two-phased multi-voting");
                }
                ...
                for (byte[] hash : hashes) {
                    ...
                    if (phasedTransactionId == 0) {
                        throw new PrizmException.NotValidException("Invalid phased transactionFullHash " + Convert.toHexString(hash));
                    }

                    PhasingPoll poll = PhasingPoll.getPoll(phasedTransactionId);
                    if (poll == null) {
                        throw new PrizmException.NotCurrentlyValidException("Invalid phased transaction " + Long.toUnsignedString(phasedTransactionId)
                                + ", or phasing is finished");
                    }
                    if (! poll.getVoteWeighting().acceptsVotes()) {
                        throw new PrizmException.NotValidException("This phased transaction does not require or accept voting");
                    }
                    long[] whitelist = poll.getWhitelist();
                    if (whitelist.length > 0 && Arrays.binarySearch(whitelist, voterId) < 0) {
                        throw new PrizmException.NotValidException("Voter is not in the phased transaction whitelist");
                    }
                    if (revealedSecret.length > 0) {
                        if (poll.getVoteWeighting().getVotingModel() != VoteWeighting.VotingModel.HASH) {
                            throw new PrizmException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " does not accept by-hash voting");
                        }
                        if (hashedSecret != null && !Arrays.equals(poll.getHashedSecret(), hashedSecret)) {
                            throw new PrizmException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " is using a different hashedSecret");
                        }
                        if (algorithm != 0 && algorithm != poll.getAlgorithm()) {
                            throw new PrizmException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " is using a different hashedSecretAlgorithm");
                        }
                        if (hashedSecret == null && ! poll.verifySecret(revealedSecret)) {
                            throw new PrizmException.NotValidException("Revealed secret does not match phased transaction hashed secret");
                        }
                        ...
                        throw new PrizmException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " requires revealed secret for approval");
                    }
                    if (!Arrays.equals(poll.getFullHash(), hash)) {
                        throw new PrizmException.NotCurrentlyValidException("Phased transaction hash does not match hash in voting transaction");
                    }
                    if (poll.getFinishHeight() <= attachment.getFinishValidationHeight(transaction) + 1) {
                        throw new PrizmException.NotCurrentlyValidException(String.format("Phased transaction finishes at height %d which is not after approval transaction height %d",
        ...
        public static final TransactionType HUB_ANNOUNCEMENT = new Messaging() {
            ...
            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingHubAnnouncement(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                if (Prizm.getBlockchain().getHeight() < Constants.TRANSPARENT_FORGING_BLOCK_7) {
                    throw new PrizmException.NotYetEnabledException("Hub terminal announcement not yet enabled at height " + Prizm.getBlockchain().getHeight());
                }
                Attachment.MessagingHubAnnouncement attachment = (Attachment.MessagingHubAnnouncement) transaction.getAttachment();
                if (attachment.getMinFeePerByteNQT() < 0 || attachment.getMinFeePerByteNQT() > Constants.MAX_BALANCE_NQT
                    ...
                    throw new PrizmException.NotValidException("Invalid hub terminal announcement: " + attachment.getJSONObject());
                }
                for (String uri : attachment.getUris()) {
                    if (uri.length() > Constants.MAX_HUB_ANNOUNCEMENT_URI_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid URI length: " + uri.length());
                    }
        ...
        public static final Messaging ACCOUNT_PROPERTY = new Messaging() {
            ...
            private final Fee ACCOUNT_PROPERTY_FEE = new Fee.SizeBasedFee(Constants.ONE_PRIZM, Constants.ONE_PRIZM, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingAccountProperty parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingAccountProperty(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountProperty parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getProperty().length() > Constants.MAX_ACCOUNT_PROPERTY_NAME_LENGTH
                    ...
                    throw new PrizmException.NotValidException("Invalid account property: " + attachment.getJSONObject());
                }
                if (transaction.getAmountNQT() != 0) {
                    throw new PrizmException.NotValidException("Account property transaction cannot be used to send coins");
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new PrizmException.NotValidException("Setting Genesis account properties not allowed");
                }
            }
        ...
        public static final Messaging ACCOUNT_PROPERTY_DELETE = new Messaging() {
            ...
            @Override
            Attachment.MessagingAccountPropertyDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.MessagingAccountPropertyDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountPropertyDelete parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (accountProperty == null) {
                    throw new PrizmException.NotCurrentlyValidException("No such property " + Long.toUnsignedString(attachment.getPropertyId()));
                }
                if (accountProperty.getRecipientId() != transaction.getSenderId() && accountProperty.getSetterId() != transaction.getSenderId()) {
                    throw new PrizmException.NotValidException("Account " + Long.toUnsignedString(transaction.getSenderId())
                ...
                if (accountProperty.getRecipientId() != transaction.getRecipientId()) {
                    throw new PrizmException.NotValidException("Account property " + Long.toUnsignedString(attachment.getPropertyId())
                ...
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new PrizmException.NotValidException("Deleting Genesis account properties not allowed");
                }
            }
    ...
    public static abstract class ColoredCoins extends TransactionType {
        ...
        public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
            ...
            private final Fee SINGLETON_ASSET_FEE = new Fee.SizeBasedFee(Constants.ONE_PRIZM, Constants.ONE_PRIZM, 32) {
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            private final Fee ASSET_ISSUANCE_FEE = (transaction, appendage) -> isSingletonIssuance(transaction) ?
                    SINGLETON_ASSET_FEE.getFee(transaction, appendage) : 1000 * Constants.ONE_PRIZM;
            ...
            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsAssetIssuance(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getName().length() < Constants.MIN_ASSET_NAME_LENGTH
                    ...
                    throw new PrizmException.NotValidException("Invalid asset issuance: " + attachment.getJSONObject());
                }
                String normalizedName = attachment.getName().toLowerCase();
                for (int i = 0; i < normalizedName.length(); i++) {
                    if (Constants.ALPHABET.indexOf(normalizedName.charAt(i)) < 0) {
                        throw new PrizmException.NotValidException("Invalid asset name: " + normalizedName);
                    }
                }
            }
        ...
        public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsAssetTransfer(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0
                    ...
                    throw new PrizmException.NotValidException("Invalid asset transfer amount or comment: " + attachment.getJSONObject());
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID && attachment.getFinishValidationHeight(transaction) > Constants.SHUFFLING_BLOCK) {
                    throw new PrizmException.NotValidException("Asset transfer to Genesis no longer allowed, "
                            + "use asset delete attachment instead");
                }
                if (transaction.getVersion() > 0 && attachment.getComment() != null) {
                    throw new PrizmException.NotValidException("Asset transfer comments no longer allowed, use message " +
                ...
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new PrizmException.NotValidException("Invalid asset transfer asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new PrizmException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
                }
            }
        ...
        public static final TransactionType ASSET_DELETE = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsAssetDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetDelete parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getAssetId() == 0) {
                    throw new PrizmException.NotValidException("Invalid asset identifier: " + attachment.getJSONObject());
                }
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new PrizmException.NotValidException("Invalid asset delete asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new PrizmException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
            ...
            @Override
            final void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                Attachment.ColoredCoinsOrderPlacement attachment = (Attachment.ColoredCoinsOrderPlacement)transaction.getAttachment();
                if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                        || attachment.getAssetId() == 0) {
                    throw new PrizmException.NotValidException("Invalid asset order placement: " + attachment.getJSONObject());
                }
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new PrizmException.NotValidException("Invalid asset order placement asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new PrizmException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        public static final TransactionType ASK_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
        ...
        public final static TransactionType BID_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
        ...
        public static final TransactionType ASK_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (ask == null) {
                    throw new PrizmException.NotCurrentlyValidException("Invalid ask order: " + Long.toUnsignedString(attachment.getOrderId()));
                }
                if (ask.getAccountId() != transaction.getSenderId()) {
                    throw new PrizmException.NotValidException("Order " + Long.toUnsignedString(attachment.getOrderId()) + " was created by account "
        ...
        public static final TransactionType BID_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (bid == null) {
                    throw new PrizmException.NotCurrentlyValidException("Invalid bid order: " + Long.toUnsignedString(attachment.getOrderId()));
                }
        ...
        public static final TransactionType DIVIDEND_PAYMENT = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                Attachment.ColoredCoinsDividendPayment attachment = (Attachment.ColoredCoinsDividendPayment)transaction.getAttachment();
                if (attachment.getHeight() > Prizm.getBlockchain().getHeight()) {
                    throw new PrizmException.NotCurrentlyValidException("Invalid dividend payment height: " + attachment.getHeight()
                            + ", must not exceed current blockchain height " + Prizm.getBlockchain().getHeight());
                }
                ...
                if (asset.getAccountId() != transaction.getSenderId() || attachment.getAmountNQTPerQNT() <= 0) {
                    throw new PrizmException.NotValidException("Invalid dividend payment sender or amount " + attachment.getJSONObject());
                }
            }
    ...
    public static abstract class DigitalGoods extends TransactionType {
        ...
        final void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
            if (transaction.getAmountNQT() != 0) {
                throw new PrizmException.NotValidException("Invalid digital goods transaction");
            }
            doValidateAttachment(transaction);
        }

        abstract void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException;


        public static final TransactionType LISTING = new DigitalGoods() {

            private final Fee DGS_LISTING_FEE = new Fee.SizeBasedFee(2 * Constants.ONE_PRIZM, 2 * Constants.ONE_PRIZM, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.DigitalGoodsListing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsListing(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsListing parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            @Override
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                Attachment.DigitalGoodsListing attachment = (Attachment.DigitalGoodsListing) transaction.getAttachment();
                if (attachment.getName().length() == 0
                    ...
                    throw new PrizmException.NotValidException("Invalid digital goods listing: " + attachment.getJSONObject());
                }
        ...
        public static final TransactionType DELISTING = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsDelisting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsDelisting(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsDelisting parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (goods != null && transaction.getSenderId() != goods.getSellerId()) {
                    throw new PrizmException.NotValidException("Invalid digital goods delisting - seller is different: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new PrizmException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
        ...
        public static final TransactionType PRICE_CHANGE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsPriceChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsPriceChange(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsPriceChange parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                    ...
                    throw new PrizmException.NotValidException("Invalid digital goods price change: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new PrizmException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
        ...
        public static final TransactionType QUANTITY_CHANGE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsQuantityChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsQuantityChange(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsQuantityChange parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getDeltaQuantity() < -Constants.MAX_DGS_LISTING_QUANTITY
                        ...
                        || (goods != null && transaction.getSenderId() != goods.getSellerId())) {
                    throw new PrizmException.NotValidException("Invalid digital goods quantity change: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new PrizmException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
        ...
        public static final TransactionType PURCHASE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsPurchase parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsPurchase(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsPurchase parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getQuantity() <= 0 || attachment.getQuantity() > Constants.MAX_DGS_LISTING_QUANTITY
                    ...
                    throw new PrizmException.NotValidException("Invalid digital goods purchase: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new PrizmException.NotValidException("Only text encrypted messages allowed");
                }
                if (goods == null || goods.isDelisted()) {
                    throw new PrizmException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
                ...
                if (attachment.getQuantity() > goods.getQuantity() || attachment.getPriceNQT() != goods.getPriceNQT()) {
                    throw new PrizmException.NotCurrentlyValidException("Goods price or quantity changed: " + attachment.getJSONObject());
                }
                if (attachment.getDeliveryDeadlineTimestamp() <= Prizm.getBlockchain().getLastBlockTimestamp()) {
                    throw new PrizmException.NotCurrentlyValidException("Delivery deadline has already expired: " + attachment.getDeliveryDeadlineTimestamp());
                }
            }

            @Override
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                if (Prizm.getBlockchain().getHeight() < Constants.MONETARY_SYSTEM_BLOCK) {
                    return false;
                }
                Attachment.DigitalGoodsPurchase attachment = (Attachment.DigitalGoodsPurchase) transaction.getAttachment();
                // not a bug, uniqueness is based on DigitalGoods.DELISTING
        ...
        public static final TransactionType DELIVERY = new DigitalGoods() {
            ...
            private final Fee DGS_DELIVERY_FEE = new Fee.SizeBasedFee(Constants.ONE_PRIZM, 2 * Constants.ONE_PRIZM, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.DigitalGoodsDelivery parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsDelivery(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsDelivery parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getGoodsDataLength() > Constants.MAX_DGS_GOODS_LENGTH) {
                    throw new PrizmException.NotValidException("Invalid digital goods delivery data length: " + attachment.getGoodsDataLength());
                }
                if (attachment.getGoods() != null) {
                    if (attachment.getGoods().getData().length == 0 || attachment.getGoods().getNonce().length != 32) {
                        throw new PrizmException.NotValidException("Invalid digital goods delivery: " + attachment.getJSONObject());
                    }
                }
                ...
                if (purchase == null || purchase.getEncryptedGoods() != null) {
                    throw new PrizmException.NotCurrentlyValidException("Purchase does not exist yet, or already delivered: "
        ...
        public static final TransactionType FEEDBACK = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsFeedback parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsFeedback(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsFeedback parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (purchase != null &&
                    ...
                    throw new PrizmException.NotValidException("Invalid digital goods feedback: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() == null && transaction.getMessage() == null) {
                    throw new PrizmException.NotValidException("Missing feedback message");
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new PrizmException.NotValidException("Only text encrypted messages allowed");
                }
                if (transaction.getMessage() != null && ! transaction.getMessage().isText()) {
                    throw new PrizmException.NotValidException("Only text public messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null) {
                    throw new PrizmException.NotCurrentlyValidException("Purchase does not exist yet or not yet delivered");
                }
            }
        ...
        public static final TransactionType REFUND = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsRefund parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.DigitalGoodsRefund(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsRefund parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getRefundNQT() < 0 || attachment.getRefundNQT() > Constants.MAX_BALANCE_NQT
                        ...
                        (purchase.getBuyerId() != transaction.getRecipientId()
                                || transaction.getSenderId() != purchase.getSellerId()))) {
                    throw new PrizmException.NotValidException("Invalid digital goods refund: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new PrizmException.NotValidException("Only text encrypted messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null || purchase.getRefundNQT() != 0) {
                    throw new PrizmException.NotCurrentlyValidException("Purchase does not exist or is not delivered or is already refunded");
                }
            }
    ...
    public static abstract class AccountControl extends TransactionType {
        ...
        public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
            ...
            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.AccountControlEffectiveBalanceLeasing(buffer, transactionVersion);
            }

            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (transaction.getSenderId() == transaction.getRecipientId()) {
                    throw new PrizmException.NotValidException("Account cannot lease balance to itself");
                }
                if (transaction.getAmountNQT() != 0) {
                    throw new PrizmException.NotValidException("Transaction amount must be 0 for effective balance leasing");
                }
                if (attachment.getPeriod() < Constants.LEASING_DELAY || attachment.getPeriod() > 65535) {
                    throw new PrizmException.NotValidException("Invalid effective balance leasing period: " + attachment.getPeriod());
                }
                byte[] recipientPublicKey = Account.getPublicKey(transaction.getRecipientId());
                if (recipientPublicKey == null && Prizm.getBlockchain().getHeight() > Constants.PHASING_BLOCK) {
                    throw new PrizmException.NotValidException("Invalid effective balance leasing: "
                            + " recipient account " + Long.toUnsignedString(transaction.getRecipientId()) + " not found or no public key published");
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new PrizmException.NotValidException("Leasing to Genesis account not allowed");
                }
            }
            ...
            void validateAttachment(Transaction transaction) throws ValidationException {
                ...
                if (votingModel == VotingModel.NONE) {
                    ...
                    if (senderAccount == null || !senderAccount.getControls().contains(ControlType.PHASING_ONLY)) {
                        throw new PrizmException.NotCurrentlyValidException("Phasing only account control is not currently enabled");
                    }
                } else if (votingModel == VotingModel.TRANSACTION || votingModel == VotingModel.HASH) {
                    throw new PrizmException.NotValidException("Invalid voting model " + votingModel + " for account control");
                }
                long maxFees = attachment.getMaxFees();
                long maxFeesLimit = (attachment.getPhasingParams().getVoteWeighting().isBalanceIndependent() ? 3 : 22) * Constants.ONE_PRIZM;
                if (maxFees < 0 || (maxFees > 0 && maxFees < maxFeesLimit) || maxFees > Constants.MAX_BALANCE_NQT) {
                    throw new PrizmException.NotValidException(String.format("Invalid max fees %f coins", ((double)maxFees)/Constants.ONE_PRIZM));
                }
                ...
                if (minDuration < 0 || (minDuration > 0 && minDuration < 3) || minDuration >= Constants.MAX_PHASING_DURATION) {
                    throw new PrizmException.NotValidException("Invalid min duration " + attachment.getMinDuration());
                }
                ...
                if (maxDuration < 0 || (maxDuration > 0 && maxDuration < 3) || maxDuration >= Constants.MAX_PHASING_DURATION) {
                    throw new PrizmException.NotValidException("Invalid max duration " + maxDuration);
                }
                if (minDuration > maxDuration) {
                    throw new PrizmException.NotValidException(String.format("Min duration %d cannot exceed max duration %d ",
    ...
    public static abstract class Data extends TransactionType {
        ...
        private static final Fee TAGGED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_PRIZM, Constants.ONE_PRIZM/10) {
            @Override
        ...
        public static final TransactionType TAGGED_DATA_UPLOAD = new Data() {
            ...
            @Override
            Attachment.TaggedDataUpload parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.TaggedDataUpload(buffer, transactionVersion);
            }

            @Override
            Attachment.TaggedDataUpload parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if (attachment.getData() == null && Prizm.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                    throw new PrizmException.NotCurrentlyValidException("Data has been pruned prematurely");
                }
                if (attachment.getData() != null) {
                    if (attachment.getName().length() == 0 || attachment.getName().length() > Constants.MAX_TAGGED_DATA_NAME_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid name length: " + attachment.getName().length());
                    }
                    if (attachment.getDescription().length() > Constants.MAX_TAGGED_DATA_DESCRIPTION_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid description length: " + attachment.getDescription().length());
                    }
                    if (attachment.getTags().length() > Constants.MAX_TAGGED_DATA_TAGS_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid tags length: " + attachment.getTags().length());
                    }
                    if (attachment.getType().length() > Constants.MAX_TAGGED_DATA_TYPE_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid type length: " + attachment.getType().length());
                    }
                    if (attachment.getChannel().length() > Constants.MAX_TAGGED_DATA_CHANNEL_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid channel length: " + attachment.getChannel().length());
                    }
                    if (attachment.getFilename().length() > Constants.MAX_TAGGED_DATA_FILENAME_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid filename length: " + attachment.getFilename().length());
                    }
                    if (attachment.getData().length == 0 || attachment.getData().length > Constants.MAX_TAGGED_DATA_DATA_LENGTH) {
                        throw new PrizmException.NotValidException("Invalid data length: " + attachment.getData().length);
                    }
                }
            }
        ...
        public static final TransactionType TAGGED_DATA_EXTEND = new Data() {
            ...
            @Override
            Attachment.TaggedDataExtend parseAttachment(ByteBuffer buffer, byte transactionVersion) throws PrizmException.NotValidException {
                return new Attachment.TaggedDataExtend(buffer, transactionVersion);
            }

            @Override
            Attachment.TaggedDataExtend parseAttachment(JSONObject attachmentData) throws PrizmException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws PrizmException.ValidationException {
                ...
                if ((attachment.jsonIsPruned() || attachment.getData() == null) && Prizm.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                    throw new PrizmException.NotCurrentlyValidException("Data has been pruned prematurely");
                }
                TransactionImpl uploadTransaction = TransactionDb.findTransaction(attachment.getTaggedDataId(), Prizm.getBlockchain().getHeight());
                if (uploadTransaction == null) {
                    throw new PrizmException.NotCurrentlyValidException("No such tagged data upload " + Long.toUnsignedString(attachment.getTaggedDataId()));
                }
                if (uploadTransaction.getType() != TAGGED_DATA_UPLOAD) {
                    throw new PrizmException.NotValidException("Transaction " + Long.toUnsignedString(attachment.getTaggedDataId())
                ...
                if (attachment.getData() != null) {
                    ...
                    if (!Arrays.equals(attachment.getHash(), taggedDataUpload.getHash())) {
                        throw new PrizmException.NotValidException("Hashes don't match! Extend hash: " + Convert.toHexString(attachment.getHash())
