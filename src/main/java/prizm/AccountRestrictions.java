...
import prizm.PrizmException.AccountControlException;
...
public final class AccountRestrictions {
    ...
    public static final class PhasingOnly {
        ...
        private void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws AccountControlException {
            if (!validatingAtFinish && maxFees > 0 && Math.addExact(transaction.getFeeNQT(), PhasingPoll.getSenderPhasedTransactionFees(transaction.getSenderId())) > maxFees) {
                throw new AccountControlException(String.format("Maximum total fees limit of %f coins exceeded", ((double)maxFees)/Constants.ONE_PRIZM));
            }
            ...
            if (!validatingAtFinish) {
                int duration = phasingAppendix.getFinishHeight() - Prizm.getBlockchain().getHeight();
    ...
    static void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws PrizmException.NotCurrentlyValidException {
        Account senderAccount = Account.getAccount(transaction.getSenderId());
        if (senderAccount == null) {
            throw new PrizmException.NotCurrentlyValidException("Account " + Long.toUnsignedString(transaction.getSenderId()) + " does not exist yet");
        }
