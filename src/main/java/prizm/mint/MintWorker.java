...
public class MintWorker {
    ...
    private void mint() {
        String currencyCode = Convert.emptyToNull(Prizm.getStringProperty("prizm.mint.currencyCode"));
        if (currencyCode == null) {
            throw new IllegalArgumentException("prizm.mint.currencyCode not specified");
        }
        String secretPhrase = Convert.emptyToNull(Prizm.getStringProperty("prizm.mint.secretPhrase", null, true));
        if (secretPhrase == null) {
            throw new IllegalArgumentException("prizm.mint.secretPhrase not specified");
        }
        boolean isSubmitted = Prizm.getBooleanProperty("prizm.mint.isSubmitted");
        boolean isStopOnError = Prizm.getBooleanProperty("prizm.mint.stopOnError");
        ...
        String unitsStr = Prizm.getStringProperty("prizm.mint.unitsPerMint");
        double wholeUnits = 1;
        ...
        long initialNonce = Prizm.getIntProperty("prizm.mint.initialNonce");
        if (initialNonce == 0) {
            initialNonce = new Random().nextLong();
        }
        int threadPoolSize = Prizm.getIntProperty("prizm.mint.threadPoolSize");
        if (threadPoolSize == 0) {
    ...
    private JSONObject mintImpl(String secretPhrase, long accountId, long units, long currencyId, byte algorithm,
        ...
        if (isSubmitted) {
            ...
            response.put("message", "prizm.mint.isSubmitted=false therefore currency mint transaction is not submitted");
        }
        return response;
    }
    ...
    private JSONObject currencyMint(String secretPhrase, long currencyId, long nonce, long units, long counter) {
        ...
        Transaction.Builder builder = Prizm.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, Constants.ONE_PRIZM,
    ...
    private JSONObject getJsonResponse(Map<String, String> params) {
        ...
        String host = Convert.emptyToNull(Prizm.getStringProperty("prizm.mint.serverAddress"));
        if (host == null) {
            try {
        ...
        boolean useHttps = Prizm.getBooleanProperty("prizm.mint.useHttps");
        if (useHttps) {
            protocol = "https";
            HttpsURLConnection.setDefaultSSLSocketFactory(TrustAllSSLProvider.getSslSocketFactory());
            HttpsURLConnection.setDefaultHostnameVerifier(TrustAllSSLProvider.getHostNameVerifier());
        }
        int port = Constants.isTestnet ? API.TESTNET_API_PORT : Prizm.getIntProperty("prizm.apiServerPort");
        String urlParams = getUrlParams(params);
        URL url;
        try {
            url = new URL(protocol, host, port, "/prizm?" + urlParams);
